...
\usepackage{balance}        % balance final two-column page
\graphicspath{{./}{final_report/}}

\title{Sentiment Analysis on Amazon Reviews: Classical, Recurrent, and Transformer-Based Models}

\author{
\IEEEauthorblockN{Nolan Capomaggio and Mathis Morra-Fischer}
\IEEEauthorblockA{Group 19\\
...
experiment, where 100,000 examples are loaded before the validation split,
Logistic Regression reaches 91.34\% test accuracy in 11.6 seconds, while the
best LSTM configuration reaches 90.71\% but requires around 9 minutes. The best
reduced RoBERTa experiment reaches 94.25\% test accuracy on a 5k-sample setup,
but uses a smaller test subset and requires substantially more runtime than
Logistic Regression.
\end{abstract}

\begin{IEEEkeywords}
sentiment analysis, Amazon reviews, TF-IDF, Logistic Regression, LSTM, RoBERTa
\end{IEEEkeywords}
...
Existing solutions for sentiment classification commonly range from sparse
linear models, such as TF-IDF with Support Vector Machines or Logistic
Regression, to recurrent neural networks and transformer encoders. Published
leaderboard-style results on the Amazon Reviews polarity task are usually high,
often above 94\% accuracy for strong models, so we use this range as a rough
upper-bound target while focusing on a fair cost/performance comparison under
our available hardware constraints.
...
In the tables below, ``loaded train examples'' refers to the value passed to
\texttt{--max\_train} before the 80/20 validation split. For example, a
100,000-example run trains on approximately 80,000 examples and validates on
approximately 20,000 examples.
...
The first important finding is that only 2,000 loaded training examples, about
1,600 actual training examples after the validation split, already give 81.45\%
test accuracy. This is above the 70--80\% threshold discussed at the midterm.
Increasing the dataset to 100,000 loaded examples improves accuracy to 91.34\%,
and the full-dataset run reached 94.07\% in 465 seconds.

\subsection{LSTM Hyperparameter Experiments}
All LSTM experiments below use 100,000 loaded training examples, which gives
about 80,000 actual training examples after the validation split, 20,000 test
...
enough capacity to adapt to the sentiment task. These runs were intentionally
performed on smaller subsets than the Logistic Regression and LSTM experiments
because RoBERTa was much more expensive to train on the available laptop.
...
training examples. This number is higher than Logistic Regression on the
100,000-loaded-example setup, but the comparison is not fully controlled:
RoBERTa is evaluated on a 2,000-example test subset, while the Logistic
Regression 100k experiment uses 20,000 test examples and the full Logistic
Regression run uses the official 400,000-example test set. Therefore, the
RoBERTa result should be interpreted as evidence that transformer fine-tuning is
promising in a reduced-data setting, not as proof that it beats Logistic
Regression on the full benchmark. It also comes at a much higher computational
cost: the 2-epoch RoBERTa run takes 707 seconds, compared with 11.57 seconds
for Logistic Regression on 100,000 loaded examples.

\section{Limitations}
RoBERTa was evaluated only on reduced training and test subsets, not on the full
dataset. This means the reported RoBERTa result should be interpreted as a
strong reduced-data experiment, not as a full benchmark of the transformer
model. The RoBERTa and Logistic Regression numbers are useful for analyzing
accuracy/runtime trade-offs, but they are not perfectly comparable unless they
use the same training size, test size, number of epochs, and hardware. Full
fine-tuning on the complete dataset would likely require more memory and more
time than was available on the laptop.
...
training examples, corresponding to about 80,000 actual training examples after
...
Adam Bittlingmayer.
\textit{Amazon Reviews for Sentiment Analysis}.
Kaggle Dataset.
